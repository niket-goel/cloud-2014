...
#### Set up the configurationDOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar $HADOOP_HOME/hadoop-0.20.2-examples.jar wordcount Data/bibtext Outputs
...
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -copyFromLocal ${HOME}/bib Data
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -ls Data/bib
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR dfs -copyFromLocal ${HOME}/word-patterns.txt /
#$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar $HADOOP_HOME/hadoop-0.20.2-examples.jar wordcount Data/bibtext Outputs
$HADOOP_HOME/bin/hadoop --config $HADOOP_CONF_DIR jar WordCountCache.jar distributedwordcount Data/bib Outputs
